...
//	private static Logger logger = LoggerFactory.getLogger(ClientGroup.class);
	...
	public static void regRemoteNode(Node remoteNode, CEPCoreClientImpl client) {
		String nodeString = remoteNode.toString();
	...
	public static void back(CEPCoreClientImpl client, Node remoteNode) {
		String nodeString = remoteNode.toString();
		List<ClientInfo> list = clients.get(nodeString);
	 ...
	 * @return
	 */
	private static CEPCoreClientImpl whenMoreThanOne(List<ClientInfo> list,
			Node remoteNode) {
		int min = -1;
	 ...
	 * @param remoteNode
	 */
	private static CEPCoreClientImpl whenOne(ClientInfo info, Node remoteNode) {
...
class ClientInfo {
	...
	public ClientInfo(CEPCoreClientImpl client) {
		super();
		this.client = client;
	}
	...
	public void setClient(CEPCoreClientImpl client) {
		this.client = client;
	}
