 ...
 */
public class TG_SDDocumentImpl extends SDDocumentSource implements SDDocument {
     ...
     *      Always non-null.
     */
    public static TG_SDDocumentImpl create(SDDocumentSource src, QName serviceName, QName portTypeName) {
        ...
        try {
            ...
            try {
                ...
                if(rootName.equals(WSDLConstants.QNAME_SCHEMA)) {
                    ...
                    return new TG_SDDocumentImpl(rootName,systemId,src);
                }
            } finally {
                reader.close();
            }
     ...
     */
    /*package*/ TG_ServiceDefinitionImpl owner;

    /**
     * The original system ID of this document.
     *
     * When this document contains relative references to other resources,
     * this field is used to find which {@link com.sun.xml.ws.server.TG_SDDocumentImpl} it refers to.
     *
     * Must not be null.
     */
    ...
    protected TG_SDDocumentImpl(QName rootName, URL url, SDDocumentSource source) {
        this(rootName, url, source, new HashSet<String>());
    }

    protected TG_SDDocumentImpl(QName rootName, URL url, SDDocumentSource source, Set<String> imports) {
        if (url == null) {
    ...
    public void writeTo(PortAddressResolver portAddressResolver, DocumentAddressResolver resolver, XMLStreamWriter out) throws XMLStreamException, IOException {
        ...
        try {
            ...
            new TG_WSDLPatcher(owner.owner,this,portAddressResolver,resolver).bridge(xsr,out);
        } finally {
            xsr.close();
        }
    }
     ...
     * @author Kohsuke Kawaguchi
     */
    private static final class SchemaImpl extends TG_SDDocumentImpl implements SDDocument.Schema {
    ...
    private static final class WSDLImpl extends TG_SDDocumentImpl implements SDDocument.WSDL {
        private final String targetNamespace;
