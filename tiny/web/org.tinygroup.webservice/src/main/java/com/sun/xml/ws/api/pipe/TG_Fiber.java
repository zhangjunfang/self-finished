 ...
 * a {@link TG_Fiber} &mdash; a user-level thread that gets created for each request/response
 * processing.
 *
 * <p>
 ...
 * <h2>Debugging Aid</h2>
 * <p>
 * Because {@link TG_Fiber} doesn't keep much in the call stack, and instead use
 ...
 *
 * <p>
 * When you debug the server side, consider setting {@link TG_Fiber#serializeExecution}
 ...
 */
public final class TG_Fiber implements Runnable {
    /**
     ...
     *  <li>"External mechanism" becomes signal state and invokes {@link TG_Fiber#resume(Packet)}
     *      to wake up fiber
     *  <li>{@link TG_Fiber#doRun} invokes {@link TG_Fiber#suspend()}.
     * </ol>
     *
     * <p>
    ...
    /**
     * Is this {@link TG_Fiber} currently running in the synchronous mode?
     */
     ...
     */
    private List<TG_FiberContextSwitchInterceptor> interceptors;
    ...
    /**
     * Callback to be invoked when a {@link TG_Fiber} finishs execution.
     */
     ...
     * {@link Tube} in the {@link TG_Fiber}'s processing stack with the specified resume 
     ...
     * on the next/first {@link Tube} in the {@link TG_Fiber}'s processing stack with
     ...
     * </ol>
     */
    public void addInterceptor(@NotNull TG_FiberContextSwitchInterceptor interceptor) {
        if(interceptors ==null) {
            interceptors = new ArrayList<TG_FiberContextSwitchInterceptor>();
     ...
     * {@link TG_Fiber#__doRun(Tube)}.
     */
    private class InterceptorHandler implements TG_FiberContextSwitchInterceptor.Work<Tube,Tube> {
        /**
         * Index in {@link TG_Fiber#interceptors} to invoke next.
         */
        private int idx;

        /**
         * Initiate the interception, and eventually invokes {@link TG_Fiber#__doRun(Tube)}.
         */
        ...
        public Tube execute(Tube next) {
            if(idx==interceptors.size()) {
            	...
            	TG_FiberContextSwitchInterceptor interceptor = interceptors.get(idx++);
                return interceptor.execute(TG_Fiber.this,next,this);
            }
        }
    }
    ...
    private Tube __doRun(Tube next) {
        final TG_Fiber old = CURRENT_FIBER.get();
     ...
     */
    public static @NotNull TG_Fiber current() {
        TG_Fiber fiber = CURRENT_FIBER.get();
        if(fiber==null)
            throw new IllegalStateException("Can be only used from fibers");
        return fiber;
    }

    private static final ThreadLocal<TG_Fiber> CURRENT_FIBER = new ThreadLocal<TG_Fiber>();
    ...
    private static final Logger LOGGER = Logger.getLogger(TG_Fiber.class.getName());
     ...
     * See class javadoc.
     */
    public static volatile boolean serializeExecution = Boolean.getBoolean(TG_Fiber.class.getName()+".serialize");
}
