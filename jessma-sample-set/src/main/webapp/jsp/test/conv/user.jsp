...
<html>
  <head>
    <base href="${__base}">
    <title>测试 Action Convention</title>
  ...
  <div align="right">
    ...
    <br>
    <div align="center">
    	<form action="test/conv/user!create.action" method="post">
    		<h2>创建新用户</h2>
    		<table>
    	...
    	<hr>
    	<form action="test/conv/user!query.action" method="post">
    		<h2>查询用户</h2>
    		<table>
    		...
    		>&nbsp;</td>
    			<td>工作年限:</td><td><select name="experience">
	    			...
	    			<c:forEach items="${__cache.experences}" var="it" varStatus="st">
	    				<option value="${it.id}"
	    					<c:if test="${__action.fromQuery && not empty __action.experience && it.id == __action.experience}">
	    						 selected="selected"
	    					</c:if>
	    				>${it.name}</option>
	    			</c:forEach>
    	...
    	<c:forEach items="${__action.users}" var="it" varStatus="st">
    		<tr align="center">
    			...
    			&nbsp;</td>
    			<td><a href="test/conv/user!delete.action?id=${it.id}">删 除</a>
    			</td>
    		</tr>
    	</c:forEach>
    	</c:if>
